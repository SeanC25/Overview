...
This .NET project ```APG_CRM``` provides a solution containing separate Data, Test and Web(MVC) projects. When installed and used to create a new project, all references to APG_CRM will be replaced with the name of your project.
...
1. An implementation of ```APG_CRM.Data.Services.IUserService``` using EntityFramework to handle data storage/retrieval is provided via ```APG_CRM.Data.Services.UserServiceDb```.
2. An implementation of ```APG_CRM.Data.Services.IMailService``` using the .NET Smtp Mail provider to handle email sending is provided via ```APG_CRM.Data.Services.SmtpMailService```.

Password hashing functionality is added via the ```APG_CRM.Data.Security.Hasher``` class. This is used in the Data project ```UserServiceDb``` to hash the user password before storing in database.
...
To install this solution as a apg_crm (apg_crm name is **termonbase**)

1. Download current version of the apg_crm

    ```$ git clone https://github.com/termon/DotNetAPG_CRMBase.git```

2. Install the apg_crm so it can be used by ```dotnet new``` command. Use the path (i.e the directory location)to the cloned apg_crm directory without trailing '/'

    Linux/macOS

    ```$ dotnet new install /path/DotNetAPG_CRMBase```

    Windows

    ```c:> dotnet new install c:\path\DotNetAPG_CRMBase```

3. Once installed you can create a new project using this apg_crm.

    ```dotnet new termonbase -n SolutionName```

4. To uninstall a apg_crm (no longer can be used with dotnet new ).

    ```dotnet new uninstall /path/DotNetAPG_CRMBase```
